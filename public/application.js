...
module.exports = {
  ...
  // host: 'http://tic-tac-toe.wdibos.com' // change to my url
...
var onSignIn = function onSignIn(event) {
  ...
  api.signIn(data).done(ui.signInSuccess).then(onCreateGame).fail(ui.signInFailure);
};
...
var onUpdateGame = function onUpdateGame(index, val, over) {
  api.updateGame(index, val, over).then(ui.updateGameSuccess).catch(ui.updateGameFailure);
};
...
var updateGame = function updateGame(index, val, over) {
  return $.ajax({
...
var getHistoryFailure = function getHistoryFailure(error) {
  console.log(error);
